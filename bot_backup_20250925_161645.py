...
def handle_account(message):
    ...
    try:
        ...
        if 'error' in balance_info:
            bot.send_message(message.chat.id, f"❌ {balance_info['error']}")
            return
        ...
        text = f"💰 *账户资产*\n"
        text += f"━━━━━━━━━━━━━━\n"
        text += f"总价值: *{balance_info['total_usdt']:.2f} USDT*\n"
        text += f"资产数: {distribution['asset_count']}\n\n"

        # 显示主要资产
        text += "*资产明细:*\n"
        for asset in distribution['distribution'][:10]:  # 只显示前10个
            emoji = "🟢" if asset['percentage'] > 10 else "🔵"
            text += f"{emoji} {asset['currency'].upper()}: "
            text += f"{asset['balance']:.6f} "
            text += f"(${asset['value_usdt']:.2f} | {asset['percentage']:.1f}%)\n"
        ...
        bot.send_message(message.chat.id, "❌ 查询账户失败，请稍后重试")
